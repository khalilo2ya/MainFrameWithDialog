...
class main(Frame):
    def __init__(self, master):
        ...
        self.master.title("Testing Dialogs - Handling data")
        self.master.geometry("300x50")
        Button(root, text="change value", command=self.dialogo).pack()
        self.valor = StringVar()
        self.valor.set("Hi Managing data")
        Label(self.master, textvariable=self.valor).pack()
 
    def dialogo(self):
        d = MyDialog(root, self.valor, "Testing Dialog", "Fill the value")
...
class MyDialog:
    def __init__(self, parent, valor, title, labeltext = '' ):
        ...
        if len(labeltext) == 0: labeltext = 'Value'
    ...
    def ok(self, event=None):
        print ("You have written ..."), self.e.get()
